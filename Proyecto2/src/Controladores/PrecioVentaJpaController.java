...
public class PrecioVentaJpaController implements Serializable {
    ...
    public void create(PrecioVenta precioVenta) {
        ...
        try {
            ...
            Producto productoId = precioVenta.getProductoId();
            if (productoId != null) {
                productoId = em.getReference(productoId.getClass(), productoId.getId());
                precioVenta.setProductoId(productoId);
            }
            em.persist(precioVenta);
            if (productoId != null) {
                productoId.getPrecioVentaCollection().add(precioVenta);
    ...
    public void edit(PrecioVenta precioVenta) throws NonexistentEntityException, Exception {
        ...
        try {
            em = getEntityManager();
            em.getTransaction().begin();
            PrecioVenta persistentPrecioVenta = em.find(PrecioVenta.class, precioVenta.getId());
            Producto productoIdOld = persistentPrecioVenta.getProductoId();
            Producto productoIdNew = precioVenta.getProductoId();
            if (productoIdNew != null) {
                productoIdNew = em.getReference(productoIdNew.getClass(), productoIdNew.getId());
                precioVenta.setProductoId(productoIdNew);
            }
            precioVenta = em.merge(precioVenta);
            if (productoIdOld != null && !productoIdOld.equals(productoIdNew)) {
                productoIdOld.getPrecioVentaCollection().remove(precioVenta);
                productoIdOld = em.merge(productoIdOld);
            }
            if (productoIdNew != null && !productoIdNew.equals(productoIdOld)) {
                productoIdNew.getPrecioVentaCollection().add(precioVenta);
    ...
    public void destroy(Integer id) throws NonexistentEntityException {
        ...
        try {
            ...
            Producto productoId = precioVenta.getProductoId();
            if (productoId != null) {
                productoId.getPrecioVentaCollection().remove(precioVenta);
                productoId = em.merge(productoId);
            }
